...
export const DataChartDeals = (backendData) => {
    ...
    for (let index = 1; index < 5; index++) {
      ...
      backendData.map((el) => {
        ...
        if (Date.parse(el.createDate) > intervalStart.getTime()
          && Date.parse(el.createDate) < intervalEnd.getTime()) contDeals += 1;
  
  
      });
      // The data is loaded into the array of objects that will be returned.
      dataChartDeal[index - 1] = { month: month, Deals_Created: contDeals }
